...
const loader = fs.readFileSync(new URL('src/boot-v8.js', root), 'utf8');
...
assert.match(html, /src="src\/boot-v8\.js"/);
assert.match(html, /updateViaCache:\s*'none'/);
assert.match(compatibilityLoader, /import '\.\/boot-v8\.js'/);
...
assert.match(serviceWorker, /black-horizon-v8/);
assert.match(serviceWorker, /src\/game\.js/);
assert.match(serviceWorker, /src\/boot-v8\.js/);
assert.match(serviceWorker, /self\.skipWaiting/);
assert.match(serviceWorker, /cache:\s*'no-store'/);
assert.match(serviceWorker, /event\.request\.mode === 'navigate'/);
assert.match(serviceWorker, /clients\.claim/);
assert.match(headers, /\/src\/boot-v8\.js/);
...
console.log('Smoke checks passed: immutable boot entrypoint, stale-shell compatibility, Cloudflare no-cache headers, immediate service-worker takeover, strict browser-safe atlas decoding, WebP integrity, production art, campaign depth, accessibility and offline shell are present.');
