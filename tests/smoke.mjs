...
for (const [key, atlas] of Object.entries(manifest.atlases)) {
  const file = new URL(atlas.src, root);
  assert.ok(fs.existsSync(file), `missing runtime atlas ${atlas.src}`);
...
assert.equal(manifest.atlases.pickups.cols * manifest.atlases.pickups.rows, 12);
...
for (const src of [manifest.screens.keyArt, manifest.screens.comicIntro, manifest.screens.heroPortrait, manifest.screens.logo]) {
  const file = new URL(src, root);
  assert.ok(fs.existsSync(file), `missing presentation vector ${src}`);
...
assert.match(serviceWorker, /src\/game\.js/);
assert.match(serviceWorker, /src\/boot-v9\.js/);
assert.match(serviceWorker, /player-atlas-v3\.svg/);
assert.match(serviceWorker, /self\.skipWaiting/);
assert.match(serviceWorker, /cache:\s*'no-store'/);
assert.match(serviceWorker, /event\.request\.mode === 'navigate'/);
assert.match(serviceWorker, /clients\.claim/);
assert.match(headers, /\/src\/boot-v9\.js/);
assert.match(headers, /assets\/sprites\/\*-v3\.svg/);
assert.match(headers, /no-cache, no-store, must-revalidate/);
...
console.log('Smoke checks passed: six versioned runtime spritesheets, 188 addressable cells, full raster integrity, campaign depth, PWA, accessibility, immutable caching and stale-shell compatibility are present.');
